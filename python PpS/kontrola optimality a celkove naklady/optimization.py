...
# COSts
ca = [800,820,730,840,850]
cb = [250,251,252,253,254]
...
prob +=pulp.lpSum(srp_vars[i]+srm_vars[i]+  pr_vars[i] + \
    mz15p_vars[i] + mz15m_vars[i] + ca[i] + cb[i]*pdg_vars[i]  for i in LOCATIONS)
...
# Suma Pdg
prob += pulp.lpSum(pdg_vars[i] for i in LOCATIONS) == Kontrakt_PDG
